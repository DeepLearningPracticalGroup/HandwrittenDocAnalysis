...
def generate_synthetic_scroll(
    ...
    for idx in range(num_images):
        ...
        max_line_height = (canvas_size[0] - 40) // num_lines  # 40 is margin at top and bottom
        ...
        for line_idx in range(num_lines):
            ...
            for _ in range(random.randint(min_chars, max_chars)):
                ...
                # Paste character
                canvas[y_cursor + line_y_offset : y_cursor + h + line_y_offset, x_cursor : x_cursor + w] = np.minimum(
                    canvas[y_cursor + line_y_offset : y_cursor + h + line_y_offset, x_cursor : x_cursor + w],
                    char_img
                )
                ...
                labels.append(f"{char_to_id[char_label]} {x_center:.6f} {y_center:.6f} {width:.6f} {height:.6f}")
