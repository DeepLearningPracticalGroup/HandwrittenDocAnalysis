...
def translate_text(text: str) -> str:
    ...
    hebrew_translation = tokenizer.decode(
...
def main(input_path: str, output_path: str):
    ...
    if not os.path.exists(input_path):
        raise FileNotFoundError(
            f"The input file '{input_path}' does not exist. Please provide a valid file path."
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Translate a .txt file from English to Hebrew."
    )
    parser.add_argument(
        "--input", type=str, required=True, help="Path to the input .txt file."
    )
    parser.add_argument(
        "--output", type=str, required=True, help="Path to save the output .txt file."
    )
